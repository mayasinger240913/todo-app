...
# מפתח ל-Claude API. מגיע ממשתנה הסביבה ANTHROPIC_API_KEY.
...
- כניסת הורה: עם אימייל וסיסמה. בפעם הראשונה ממלאים הכל, ובפעמים הבאות האימייל נזכר
  אוטומטית וצריך רק סיסמה. הסיסמה חייבת להכיל אותיות וגם מספרים (לפחות 6 תווים).
- שכחת סיסמה: לוחצים "שכחתי סיסמה", מקבלים קוד בן 6 ספרות לאימייל, ומזינים אותו כדי
  לבחור סיסמה חדשה.
- כניסת ילדים: בלי שום קוד — פשוט לוחצים על הפרופיל (התמונה/האימוג'י) של הילד.
- לכל משפחה יש קישור ייחודי משלה בצורת /f/<קוד-משפחה>. ההורה משתף את הקישור הזה עם
  בני המשפחה כדי שהילדים ייכנסו. הכתובת הראשית משמשת רק לכניסה/הרשמה.
- ההורה יכול: להוסיף/לערוך/למחוק ילדים (כולל שינוי שם ואימוג'י), מטלות ופרסים;
  לאשר או לדחות ביצוע מטלות ובקשות פרסים; לתת בונוס נקודות; ולראות התראות.
- מטלות לדוגמה: הכנסת/הוצאת כלים מהמדיח, סידור הסלון/המטבח, שטיפת רצפות, הכנת ארוחת
  ערב, עזרה לאח/אחות בשיעורי בית. יש גם כפתור שבו ילד מבקש מטלה חדשה מההורה.
- צילום הוכחה: חייבים לצלם בזמן אמת דרך המצלמה (לא להעלות מהגלריה). דרוש אישור גישה
  למצלמה ואתר מאובטח (HTTPS).
- בדיקת הוגנות (Anti-Cheat): כל תמונה נבדקת אוטומטית (טביעת אצבע ויזואלית + מטא-דאטה)
  כדי לזהות חשד לתמונה ישנה/מועלית/צילום-מסך. בעת חשד נשלחת התראה *רק להורה* (הילד
  לא מקבל הודעה), וההורה מחליט בעצמו אם להפחית נקודות.
- מחירים: 5 מטלות חינם בכל חודש, ולאחר מכן מנוי של 19.90 ₪ לחודש למספר מטלות בלתי מוגבל.
- אפשר להתקין את האפליקציה בטלפון (PWA) דרך "הוסף למסך הבית" בדפדפן.

כללי מענה:
- ענה/י תמיד בעברית, בחום ובפשטות, במשפטים קצרים וברורים.
- אם המשתמש שואל "איך עושים X" — תן/י צעדים קצרים וממוספרים.
- אם אינך יודע/ת משהו או שזה לא קשור לאפליקציה — אמור/אמרי זאת בכנות והצע/י לפנות
  לתמיכה במייל todo.family.app1@gmail.com.
- אל תמציא/י תכונות שלא תוארו כאן. אל תבקש/י סיסמאות או פרטים רגישים."""
...
def chat():
    """עוזר AI שעונה על שאלות לגבי האפליקציה.
    מקבל מהלקוח רשימת הודעות (היסטוריה) ומחזיר תשובה."""
    # אם אין מפתח API — הצ'אט לא מוגדר עדיין. מחזירים הודעה ידידותית (לא שגיאה).
    if not ANTHROPIC_API_KEY:
        return jsonify({
            "reply": "העוזר החכם עדיין לא הופעל 🛠️ אפשר לפנות בינתיים לתמיכה "
                     "במייל todo.family.app1@gmail.com ונשמח לעזור!",
            "configured": False,
        })
    ...
    if not isinstance(raw_messages, list) or not raw_messages:
        return jsonify({"error": "לא נשלחה הודעה"}), 400

    # מנקים ומאמתים את ההודעות מהלקוח (רק user/assistant, תוכן טקסט קצר).
    messages = []
    for m in raw_messages[-12:]:  # שומרים רק את 12 ההודעות האחרונות
        role = m.get("role")
    ...
    try:
        ...
        return jsonify({
            "reply": "העוזר החכם זמנית לא זמין. נסו שוב מאוחר יותר 🙏",
            "configured": False,
        })
    ...
    try:
        ...
        response = client.messages.create(
            model=CHAT_MODEL,
            max_tokens=600,
            # שומרים את הוראת המערכת במטמון כדי לחסוך בעלות ובזמן בכל שיחה.
            system=[{
                "type": "text",
        ...
        if not reply:
            reply = "מצטער/ת, לא הצלחתי לנסח תשובה. אפשר לנסות לשאול שוב?"
        return jsonify({"reply": reply, "configured": True})
    except anthropic.AuthenticationError:
        print(">> צ'אט: מפתח ANTHROPIC_API_KEY שגוי")
        return jsonify({
            "reply": "יש בעיה בהגדרת העוזר החכם. אנא פנו לתמיכה 🙏",
            "configured": False,
        })
    except anthropic.RateLimitError:
        return jsonify({
            "reply": "העוזר עמוס כרגע 😅 נסו שוב בעוד רגע.",
            "configured": True,
        })
    except Exception as e:
        print(">> שגיאת צ'אט:", e)
        return jsonify({
            "reply": "אופס, משהו השתבש. נסו שוב בעוד רגע 🙏",
            "configured": True,
        })
